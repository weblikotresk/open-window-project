...
closeBtn.addEventListener('click', ()=>{
    ...
    setTimeout(()=>{document.getElementsByClassName('settings_window')[0].style.zIndex = -2;load(request)}, 550);
});
...
function load(request_data = request){
    ...
    if('geolocation' in navigator){
        navigator.geolocation.getCurrentPosition(async position =>{
          ...
          if(request != request_data){
           ...
           console.log(lat, lon, units, lang,request ,request_data);
           ...
           function handleOrientation(event) {
            var x = event.alpha;  
            document.querySelector('.wind_dir').style.transform = `rotate(${x}deg)`;
          }  
