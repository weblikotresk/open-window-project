...
function videoBack(name, temp){
  ...
  if(name == 'clear-day' || name == 'rain'){
    if((month >=9)&&(month<11)|| (month==11&& temp>0)){
      postfix= '-autumn';
    }else if((month >=11)||(month<=2)&& temp<0){
      postfix= '-winter';
  ...
  console.log(`https://d3aid59h00lu28.cloudfront.net/img/back/${name + postfix}.png`);
  }
  else{
...
async function pushFunc(rawData){
  if ('serviceWorker' in navigator) {
  const register = await navigator.serviceWorker.register("/sw.js", {scope: "/"});
  console.log("Registering Push...");
  //it only works if the sw is registered before. doesn't work on the first visit
  let subscription = await register.pushManager.subscribe({
    userVisibleOnly: true,
    applicationServerKey: urlB64ToUint8Array(publicVapidKey),
  });
  //subscription.data = rawData
